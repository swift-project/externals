...
This file is placed in the "C:\Users\<username>\AppData\Roaming\Lockheed Martin\Prepar3D v2"
directory - the same directory where the Prepar3D.cfg file is located. This file does not need to be
present if the SimConnect system is used in the default configuration that supports all SimConnect
clients that are running on the same machine that Prepar3D is running on. Only if it is desired to have
...
placed in the "C:\Users\<username>\AppData\Roaming\Lockheed Martin\Prepar3D v2" directory - the
same directory where the Prepar3D.cfg file is located.